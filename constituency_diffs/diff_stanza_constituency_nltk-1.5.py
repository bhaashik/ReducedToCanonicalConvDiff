    ...
    with open(filename, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line_strip.startswith("(sentence"):
                # If tree accumulated, save it first
                if current_lines:
                    tree_str = "\n".join(current_lines)
                    try:
            ...
            if not line_strip:
                # Blank line signals tree boundary
                if current_lines:
                    tree_str = "\n".join(current_lines)
                    try:
                        tree = Tree.fromstring(tree_str)
...
# --- Updated word_context: track parents during traversal ---

def word_context(tree, target_word):
    """
    Return list of contexts for target_word containing:
    pos tag, parent label, grandparent label, and siblings' leaves.
    """
    contexts = []
    ...
    #         return
    #     for child in t:
    #         if child.height() == 2 and target_word in child.leaves():
    #             siblings = [sib for sib in t if sib != child]
    #             context = {
    #                 "pos": child.label(),
    #                 "parent": t.label(),
    #                 "grandparent": parent_label,
    #                 "siblings": [sib.leaves() for sib in siblings]
    #             }
...
# --- Accumulate global event statistics ---
...
def accumulate_global_stats(all_events):
    ...
    # Set what can't be logically inferred to zero
