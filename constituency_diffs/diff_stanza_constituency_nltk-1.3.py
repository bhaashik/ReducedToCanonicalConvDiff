...
# def parse_constituent_trees(tree_file):
#     with open(tree_file, "r", encoding="utf-8") as f:
#         return [Tree.fromstring(line.strip()) for line in f if line.strip()]

# def read_constituency_file_with_metadata(tree_file):
#     meta_lines = []
#     trees = []
#     with open(tree_file, "r", encoding="utf-8") as f:
#         for line in f:
#             line = line.strip()
#             if line.startswith('(') and not line.startswith('(sentence'):
#                 try:
#                     tree = Tree.fromstring(line)
#                     trees.append(tree)
#                 except Exception as e:
#                     print(f"Skipping malformed tree line: {line[:60]}... due to {e}")
#             elif line.startswith('(sentence'):
#                 meta_lines.append(line)
#     return meta_lines, trees

def read_trees_with_metadata(filename):
    """
    Reads a file with constituency parses and interlaced metadata lines.
    Metadata lines (e.g. '(sentence 1.1) ...') are mapped to the following tree.

    Args:
        filename (str): Path to the file with bracketed parse trees and metadata.

    Returns:
        List of tuples: [(metadata_line_or_None, nltk.Tree), ...]
    """
    result = []
    current_lines = []
    ...
    with open(filename, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Metadata lines usually start with '(sentence' or similar, adjust pattern if needed
            if line_strip.startswith("(sentence"):
                # If there was a previous unfinished tree, parse and store it first
                if current_lines:
                    tree_str = "\n".join(current_lines)
                    try:
                ...
                # Save this line as the current metadata
                current_metadata = line_strip
                continue

            # Blank lines signal boundaries; parse tree if accumulated
            if not line_strip:
                if current_lines:
                    tree_str = "\n".join(current_lines)
                    try:
            ...
            # All other lines (likely parse tree lines) collect here
            if line_strip.startswith("("):
                current_lines.append(line.rstrip())

        # At EOF, handle last tree if any
        if current_lines:
            tree_str = "\n".join(current_lines)
            try:
                tree = Tree.fromstring(tree_str)
...
def constituent_spans(tree):
    ...
    def helper(t, start, parent_label):
        ...
        if t.height() > 2:
            words_in_span = t.leaves()
            spans.append((start, end, t.label(), parent_label, words_in_span))
        return end
    helper(tree, 0, None)
    return spans
...
def word_context(tree, target_word):
    ...
    for subtree in tree.subtrees():
        if subtree.height() == 2 and target_word in subtree.leaves():
            parent = subtree.parent()
            grandparent = parent.parent() if parent is not None else None
            siblings = parent[:] if parent is not None else []
            context = {
                "pos": subtree.label(),
                "parent": parent.label() if parent else None,
                "grandparent": grandparent.label() if grandparent else None,
                "siblings": [sib.leaves() for sib in siblings if sib != subtree]
            }
...
def compare_pair(head_tree, canon_tree):
    ...
    word_to_pos_h = dict((w, pos) for w, pos, _ in h_words)
    word_to_pos_c = dict((w, pos) for w, pos, _ in c_words)
    changed_subset = (h_word_set & c_word_set)
    pos_changes = []
    for w in changed_subset:
...
def export_local_events_to_csv(all_events, filename):
    fieldnames = ["pair_index", "event_type", "word/constituent", "pos_label", "parent_label", "grandparent_label", "additional_context"]
    with open(filename, "w", newline='', encoding='utf-8') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
...
def main():
    headline_file = "/mnt/d/projects/Bhaashik/ReducedToCanonicalConvDiff/Hindustan-Times-headlines-stanza-parsed-constituency.txt"
    canonical_file = "/mnt/d/projects/Bhaashik/ReducedToCanonicalConvDiff/Hindustan-Times-canonical-stanza-parsed-constituency.txt"

    # headline_trees = parse_constituent_trees(headline_file)
    # canonical_trees = parse_constituent_trees(canonical_file)

    meta_tree_pairs_headline = read_trees_with_metadata(headline_file)
    print(f"Read {len(meta_tree_pairs_headline)} headline trees with metadata.")

    meta_tree_pairs_canonical = read_trees_with_metadata(canonical_file)
    print(f"Read {len(meta_tree_pairs_canonical)} canonical trees with metadata.")

    # Example usage:
    # if __name__ == "__main__":
    #     filename = "your_parse_file.txt"
    #     meta_tree_pairs = read_trees_with_metadata(filename)
    #     print(f"Read {len(meta_tree_pairs)} trees with metadata.")
    # 
    #     # Access example
    #     for meta, tree in meta_tree_pairs[:3]:  # first 3 items
    #         print("Metadata:", meta)
    #         print("Tree:", tree)
    #         print("---")

    assert len(meta_tree_pairs_headline) == len(meta_tree_pairs_canonical), "Tree count mismatch!"

    all_events = []
    for h_tree, c_tree in zip(meta_tree_pairs_headline, meta_tree_pairs_canonical):
        all_events.append(compare_pair(h_tree, c_tree))
