...
# --- Parsing multi-line constituency trees with metadata ---

def read_trees_with_metadata(filename):
    """
    Reads constituency parse file with metadata lines like '(sentence 1.1) ...'
    Maps metadata line to the following tree.
    Returns list of tuples: (metadata_line or None, nltk.Tree)
    """
    result = []
    current_lines = []
    ...
    with open(filename, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # Metadata line detection (adjust if metadata format differs)
...
# --- Tree and event helper functions ---

def get_word_pos_lists(tree):
    # Returns list of (word, POS_tag, index)
    return [(w, pos, i) for i, (w, pos) in enumerate(tree.pos())]

def constituent_spans(tree):
    # Returns list of (start, end, label, parent_label, words)
    spans = []
...
def word_context(tree, target_word):
    # Returns list of dicts with pos, parent, grandparent, siblings of given word
    contexts = []
...
# --- Compare one pair of constituency trees ---
...
# --- Statistical testing and counts accumulation ---
...
def accumulate_global_stats(all_events):
    ...
    # For events without logically inferable counts on both sides:
