...
def align_tokens(can_tokens, red_tokens, match_score=2, mismatch_score=-1, gap_score=-2):
    ...
    # Use 'first' and 'second' attributes for alignment indices
    alignment_path = [
...
def extract_diffs(can_tokens, red_tokens, alignment_path):
    ...
    for can_idx, red_idx in alignment_path:
        ...
        if can_idx is not None and red_idx is not None:
            ...
            # Map canonical head token index to headline token index
            if c_head == 0 and r_head == 0:
                pass  # both roots, no diff
            else:
        ...
        # Handle omissions (canonical token aligned to gap)
        ...
        # Handle insertions (headline token aligned to gap)
...
def main(headlines_path, canonical_path, output_csv="pairwise_diffs.csv"):
    ...
    for idx, (head_sent, can_sent) in enumerate(zip(headline_sents, canonical_sents), start=1):
        ...
        # Check for any headline index out of range before extract_diffs
        for can_idx, red_idx in alignment_path:
            if red_idx is not None and (red_idx < 0 or red_idx >= len(red_tokens)):
                pair_id = f"sent{idx:05d}"
                debug_alignment_issue(pair_id, can_tokens, red_tokens, alignment_path)
                break

        diffs = extract_diffs(can_tokens, red_tokens, alignment_path)

        # Track global frequencies
...
def debug_alignment_issue(pair_id, can_tokens, red_tokens, alignment_path):
    """
    Print detailed info for one sentence pair when headline index out-of-range warnings occur.
    """
    print(f"\n=== DEBUG: Problematic sentence pair {pair_id} ===")
    print(f"Canonical sentence tokens (count={len(can_tokens)}):")
    print([tok['form'] for tok in can_tokens])
    print(f"Headline sentence tokens (count={len(red_tokens)}):")
    print([tok['form'] for tok in red_tokens])

    print(f"\nAlignment path (showing first 30 pairs):")
    for i, (can_idx, red_idx) in enumerate(alignment_path[:30]):
        print(f"  [{i}] Canonical idx: {can_idx}, Headline idx: {red_idx}")

    # Print any headline indices out of range
    for can_idx, red_idx in alignment_path:
        if red_idx is not None:
            if red_idx < 0 or red_idx >= len(red_tokens):
                print(f"WARNING: headline index {red_idx} out of range! Headline token count: {len(red_tokens)}")
                # Optionally print more detail and break to avoid flooding
                break
