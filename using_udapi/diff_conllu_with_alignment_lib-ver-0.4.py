...
def align_tokens(can_tokens, red_tokens, match_score=2, mismatch_score=-1, gap_score=-2):
    ...
    Returns: alignment path - list of (can_idx or None, red_idx or None).
    """
    vocab = Vocabulary()
    ...
    # Alignments is a list of SequenceAlignment objects; take first (best) alignment
    alignment = alignments[0]

    # alignment.alignment is a list of tuples (idx_in_can or -1, idx_in_red or -1)
    # Convert -1 to None for clarity
    #alignment_path = [(i if i != -1 else None, j if j != -1 else None) for i, j in alignment.alignment]
    #print(dir(alignment))
    #alignment_path = [(i if i != -1 else None, j if j != -1 else None) for i, j in alignment.aligned]
    alignment_path = [
...
def extract_diffs(can_tokens, red_tokens, alignment_path):
    """
    Given tokens and the alignment path, extract key differences:
    omissions, insertions, POS differences, deprel differences, head differences,
    feats differences, token reorderings.
    """
    diffs = {
        'omitted_in_headline': [],
    ...
    for can_idx, red_idx in alignment_path:
        if can_idx is not None and red_idx is not None:
            ...
            # Compare heads after mapping canonical head token idx with headline
            c_head = c['head']
            r_head = r['head']

            # Map canonical head (1-based token id) to headline head index if possible
            if c_head == 0 and r_head == 0:
                ...
                if mapped_head is None:
                    # Head maps to missing token -> difference
                    diffs['head_diff'].append((c['form'], c_head, r_head))
                else:
                    # Compare mapped head position with headline head id (headline tokens are 1-based)
